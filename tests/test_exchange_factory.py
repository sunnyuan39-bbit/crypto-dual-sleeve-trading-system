...
def test_exchange_factory_uses_inert_without_keys() -> None:
    config = RuntimeConfig(TradingMode.SIGNAL_ONLY, ExchangeName.BINANCE_USDM_TESTNET)
...
def test_exchange_factory_requires_keys_when_fallback_disabled() -> None:
    config = RuntimeConfig(TradingMode.EXCHANGE_TESTNET, ExchangeName.BINANCE_USDM_TESTNET)

    with pytest.raises(MissingTestnetCredentials):
        build_exchange_adapter(config, allow_inert_fallback=False)


def test_exchange_factory_builds_real_adapter_with_keys() -> None:
    config = RuntimeConfig(
        TradingMode.EXCHANGE_TESTNET,
        ExchangeName.BINANCE_USDM_TESTNET,
        api_key="key",
        api_secret="secret",
    )
